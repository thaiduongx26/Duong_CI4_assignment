...
import vn.edu.techkids.controllers.Gift.GiftItemController;
import vn.edu.techkids.controllers.Gift.GiftItemType;
...
//    private BulletController bulletController;
    ...
    public boolean checkGiftBullet = false;
    ...
    public void shot() {
        if (checkGiftBullet == true) {

            if (bulletControllerManager.size() < MAX_BULLET_COUNT) {
                this.setBullet(2*BulletController.SPEED_DEFAULT,
                        2*Bullet.DAMAGE_DEFAULT);
            }
        } else {
            if (bulletControllerManager.size() < MAX_BULLET_COUNT) {
                this.setBullet(BulletController.SPEED_DEFAULT,
                        Bullet.DAMAGE_DEFAULT);
            }
        }
    }
    ...
    public void onCollide(Colliable c) {
        ...
        } else if (c instanceof GiftItemController){
            c.getGameObject().setAlive(false);
//            System.out.println(GiftItemController.giftType);
            switch (GiftItemController.giftType){
                case BOMB:
                    for (Colliable colliable : CollisionPool.getInst().getColliableVector()){
                        if (colliable instanceof EnemyPlaneController){
...
//                            System.out.println("yes");
                    ...
                    break;
                case BULLET:
                    this.checkGiftBullet = true;
//                    Bullet bullet = (Bullet)gameObject;
//                    bullet.setDamage(2*Bullet.DAMAGE_DEFAULT);
                    break;
            }
        }
    }
